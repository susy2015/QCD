...
void plotsearchbin37_mod(const TString &fileName = "MCmix_ClosureNewbin.root",
		     const TString &graphicsFormat = "pdf") {
  gROOT->ProcessLine(".L HistStyle.h+");
  gROOT->ProcessLine(".L HistReader.h+");
  //gROOT->ProcessLine(".L searchBins.h+");
  HistStyle::init();
  int canSizeX = 600;
  int canSizeY = 600;
  ...
  double ymax_Ratio = 20.0;
  double ymin_Ratio = 0.05;
  ...
  for(unsigned int i = 0; i < kNDists; ++i) {
    ...
    //    else if (i == 10) name = "mht";
    
    hPred[i] = HistReader::get("../../../37Result/PredQCDMC.root","h_pred_sb");
    hTrue[i] = HistReader::get("../../../37Result/ExpQCD.root","h_exp_sb");
    
    //hPred[i] = HistReader::get("PredQCDMC.root","h_pred_sb");
    //hTrue[i] = HistReader::get("ExpQCD.root","h_exp_sb");
  }
  ...
  TH1* hRatio[kNDists];
  //KH-starts
  TH1* hDenominator[kNDists];
  TH1* hDenominator_NoError[kNDists];
  //KH-ends
  for(unsigned int i = 0; i < kNDists; ++i) {
    hRatio[i] = static_cast<TH1*>(hTrue[i]->Clone("Ratio"));
    // KH-starts
    hDenominator_NoError[i] = static_cast<TH1*>(hPred[i]->Clone("hDenominator_NoError"));
    hDenominator[i] = static_cast<TH1*>(hPred[i]->Clone("hDenominator"));
    for (int ibin=0; ibin<hDenominator_NoError[i]->GetNbinsX()+2; ibin++){
      // scan including underflow and overflow bins
      hDenominator_NoError[i]->SetBinError(ibin,0.);
    }
    hRatio[i]->Divide(hDenominator_NoError[i]);    
    hDenominator[i]->Divide(hDenominator_NoError[i]);
    //hRatio[i]->GetYaxis()->SetTitle("#frac{Prediction}{Direct}");
    //KH-ends
    hRatio[i]->GetYaxis()->SetTitle("#frac{Direct}{Prediction}");
    hRatio[i]->GetYaxis()->SetRangeUser(0.05,20.0);
    hRatio[i]->SetTitle("");
    hRatio[i]->SetStats(0);
    hRatio[i]->SetLineWidth(1);
    hRatio[i]->GetYaxis()->SetTitleSize(0.15);
    hRatio[i]->GetYaxis()->SetTitleOffset(0.3);
    hRatio[i]->GetYaxis()->SetTitleFont(62);
    hRatio[i]->GetYaxis()->SetLabelSize(0.1);
    hRatio[i]->GetYaxis()->SetLabelFont(62);
    hRatio[i]->GetXaxis()->SetLabelOffset(0.01);
    hRatio[i]->GetXaxis()->SetLabelFont(62);    
    hRatio[i]->GetXaxis()->SetLabelSize(0.08);
    hRatio[i]->GetXaxis()->SetTitleSize(0.16);
    hRatio[i]->GetXaxis()->SetTitleFont(62);
    hRatio[i]->GetXaxis()->SetTitleOffset(0.6);
    if (i == 0){
      hRatio[i]->GetXaxis()->SetTitle("Search region bin number");
      hRatio[i]->SetAxisRange(0,37,"X");
      hRatio[i]->SetAxisRange(0.05,20.0,"Y");
    }
  ...
  // Create legend
  TLegend* leg = new TLegend(0.54,0.62,0.93,0.85);
  leg->SetBorderSize(1);
  leg->SetLineColor(1);
  leg->SetLineWidth(2);
  leg->SetFillColor(0);
  ...
  //KH leg->AddEntry(hPred[0],"Treat simulation like data","L");
  ...
  */
  ...
  for(unsigned int i = 0; i < kNDists; ++i) {
    ...
    pad1->SetTopMargin(0.12); // Upper and lower plot are joined
    pad1->SetBottomMargin(0); // Upper and lower plot are joined
    pad1->SetLeftMargin(0.10);  // Reduce the left margin
    pad1->SetRightMargin(0.05); // Reduce the right margin
    pad1->Draw();             // Draw the upper pad: pad1
    pad1->cd();               // pad1 becomes the current pad

    TExec * setex2 = new TExec("setex2", "gStyle->SetErrorX(0.0)");
    TExec *setex = new TExec("setex", "gStyle->SetErrorX(0.5)");
    hTrue[i]->GetXaxis()->SetRangeUser(0., 37.);
    setex2->Draw();
    hTrue[i]->Draw("PE1");
    setex->Draw();
    ...
    if (i==0){
      ...
      //drawSBregionDef(ymin_Yields,ymax_Yields,0.070,true);
      ...
      ttext_ntop->DrawLatex(10.5, ymax_Yields/3.5 ,"N_{top} = 1");
      ttext_ntop->DrawLatex(29.0, ymax_Yields/250. ,"N_{top} #geq 2");
      ...
      ttext2->DrawLatex( 5.5, ymax_Yields/10., "N_{b} = 1");
      ttext2->DrawLatex(16.0, ymax_Yields/10., "N_{b} #geq 2");
      ttext2->DrawLatex(25.0, ymax_Yields/800., "N_{b} = 1");
      ttext2->DrawLatex(33.0, ymax_Yields/800., "N_{b} #geq 2");
      ...
      tl_mt2->DrawLine(24,ymin_Yields,24,ymax_Yields/2000.); 
      tl_mt2->DrawLine(27,ymin_Yields,27,ymax_Yields/2000.); 
      tl_mt2->DrawLine(32,ymin_Yields,32,ymax_Yields/2000.);
      tl_mt2->DrawLine(35,ymin_Yields,35,ymax_Yields/2000.);
      ...
      ttextmt2->DrawLatex( 2.5, ymax_Yields/5000. , "M_{T2}=[200,300]");
      ttextmt2->DrawLatex( 6.0, ymax_Yields/5000. , "M_{T2}=[300,400]");
      ttextmt2->DrawLatex( 9.5, ymax_Yields/5000. , "M_{T2}#geq400 GeV");
      //-----------------------------------------------------------
    ...
    pad2->SetLogy();
    //if(i==0) pad2->SetGridy(); 
    hRatio[i]->SetMarkerSize(1);
    hRatio[i]->SetMaximum(20.0);
    hRatio[i]->SetMinimum(0.05);
    if(i==0){
      hRatio[i]->GetXaxis()->SetLabelSize(0.1);
      hRatio[i]->GetXaxis()->SetTitleOffset(0.8);
    }

    hRatio[i]->GetXaxis()->SetRangeUser(0., 37.);
    hRatio[i]->Draw("PE1");
    //KH hRatioFrame[i]->Draw("HISTsame");
    //KH-srarts
    setex->Draw();
    hDenominator[i]->DrawCopy("histsame");
    hDenominator[i]->SetFillColor(kBlue-4);
    hDenominator[i]->SetFillStyle(3001);
    hDenominator[i]->Draw("E2same");    
    hDenominator[i]->Print("all");
    setex2->Draw();
    //KH-ends
