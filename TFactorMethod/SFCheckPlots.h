...
void SFCheckPlots::Initialization(std::string dir)
{
  target_DIR = dir;
...
void SFCheckPlots::PrintPlotsName()
{
...
void SFCheckPlots::BasicCheckTemplate(
                                         TString hist_tag,
                                         TString XTitle,
                                         double min,
                                         double max
...
{ 
  ...
  for(int i  = 0 ; i < NHist ; i++)
  {
    ...
    if( TString(list->At(i)->GetName()).Contains( hist_tag ) )
    {
      if( TString(list->At(i)->GetName()).Contains( "_MC" ) )
      {
        ...
        if( TString(list->At(i)->GetName()).Contains( "ST" ) )     { smalltag = "ST";     leg->AddEntry( (TH1D*)fin->Get(list->At(i)->GetName()), smalltag.c_str(), "f"); }
  ...
  c->SaveAs( target_DIR + TString("/") + hist_tag + TString("_SFCheck.png") );
  c->SaveAs( target_DIR + TString("/") + hist_tag + TString("_SFCheck.pdf") );
  c->SaveAs( target_DIR + TString("/") + hist_tag + TString("_SFCheck.C") );
}
