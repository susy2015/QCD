...
void drawSBregionDef(const double ymin_Yields = 0.05, const double ymax_Yields = 500., const bool logscale=true)
{
  ...
  if(logscale)
  {
    if(NSB == 45)
    {
	    ttext_ntop->DrawLatex(11.5 + adjHalfBin, ymax_Yields/1.2 ,"N_{top} = 1");
	    ttext_ntop->DrawLatex(35.5 + adjHalfBin, ymax_Yields/6. ,"N_{top} = 2");
  ...
  if(logscale)
  {
    ttext2->DrawLatex( 4.5 + adjHalfBin, ymax_Yields/2., "N_{b} = 1");
    ttext2->DrawLatex(14.5 + adjHalfBin, ymax_Yields/2., "N_{b} = 2");
    if(NSB == 45)
    {
	    ttext2->SetTextAngle(90.);
	    ttext2->DrawLatex(21.5 + adjHalfBin, ymax_Yields/2. , "N_{b} #geq 3");
    }
  } 
	else
  {
  ...
  if(logscale)
  {
    ttextmt2->DrawLatex( 1.5 + adjHalfBin, ymax_Yields/4. , "M_{T2}=[200,300]");
    ttextmt2->DrawLatex( 5.0 + adjHalfBin, ymax_Yields/4. , "M_{T2}=[300,400]");
    ttextmt2->DrawLatex( 8.5 + adjHalfBin, ymax_Yields/4. , "M_{T2}#geq400");
  } 
	else
  {
