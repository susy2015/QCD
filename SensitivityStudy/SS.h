...
void SSDataCard::printDC_AllFiles()
{
  ...
  */
  ...
  if (LLfile.is_open())
  {
    ...
    LLfile << "channel = ";
    for(int i=0;i<NSB;i++){ LLfile << "bin" << i+1 << " "; }
    LLfile << "\n";

    LLfile << "# Predicted central numbers (need from all backgrounds)\n";
    LLfile << "rate = ";
    for(int i=0;i<NSB;i++){ LLfile << DC_sb_MC_LL[i] << " "; }
    LLfile << "\n";

    LLfile << "# Control sample events for lost lepton; Raw MC yields for Zinv and ttZ; No need from had. tau and QCD (will be ignored).\n";
    LLfile << "cs_event = ";
    for(int i=0;i<NSB;i++){ LLfile << DC_sb_MC_LL_cs[i] << " "; }
    LLfile << "\n";

    LLfile << "# Average weight for lost lepton, Zinv and ttZ: avg_weight x cs_event = rate. Will be ignored for had. tau and QCD.\n";
    LLfile << "avg_weight = ";
    for(int i=0;i<NSB;i++){ LLfile << DC_sb_MC_LL_avgweight[i] << " "; }
    LLfile << "\n";

    LLfile << "# All the uncertainties in absolute numbers for both stat and syst uncertainties. Needed from QCD and hadronic tau; No need from lost lepton, Zinv and ttZ (will be ignored). For symmetric uncertainties, put the up and dn to be the same.\n";
    LLfile << "stat_unc_up = ";
    for(int i=0;i<NSB;i++){ LLfile << /*DC_sb_MC_LL_statunc[i]*/"No Need!!" << " "; break; }
    LLfile << "\n";
    LLfile << "stat_unc_dn = ";
    for(int i=0;i<NSB;i++){ LLfile << /*DC_sb_MC_LL_statunc[i]*/"No Need!!" << " "; break; }
    LLfile << "\n";

    LLfile << "# List of all the systematical uncertainties. Do not need combine them. The \"pdf\", \"blackhole\", \"darkmatter\", \"susy\" are keywords to label the different systematic sources (use meaningful names or make comments).\n";
    LLfile << "syst_unc_all_up = ";
    for(int i=0;i<NSB;i++){ LLfile << DC_sb_MC_LL_systunc[i] << " "; }
    LLfile << "\n";
    LLfile << "syst_unc_all_dn = ";
    for(int i=0;i<NSB;i++){ LLfile << DC_sb_MC_LL_systunc[i] << " "; }
    LLfile << "\n";
    LLfile.close();
  }
  ...
  if (HadTaufile.is_open())
  {
    ...
    HadTaufile << "channel = ";
    for(int i=0;i<NSB;i++){ HadTaufile << "bin" << i+1 << " "; }
    HadTaufile << "\n";

    HadTaufile << "# Predicted central numbers (need from all backgrounds)\n";
    HadTaufile << "rate = ";
    for(int i=0;i<NSB;i++){ HadTaufile << DC_sb_MC_HadTau[i] << " "; }
    HadTaufile << "\n";

    HadTaufile << "# Control sample events for lost lepton; Raw MC yields for Zinv and ttZ; No need from had. tau and QCD (will be ignored).\n";
    HadTaufile << "cs_event = ";
    for(int i=0;i<NSB;i++){ HadTaufile << /*DC_sb_MC_HadTau_cs[i]*/"No Need!!" << " "; break; }
    HadTaufile << "\n";

    HadTaufile << "# Average weight for lost lepton, Zinv and ttZ: avg_weight x cs_event = rate. Will be ignored for had. tau and QCD.\n";
    HadTaufile << "avg_weight = ";
    for(int i=0;i<NSB;i++){ HadTaufile << /*DC_sb_MC_HadTau_avgweight[i]*/"No Need!!" << " "; break; }
    HadTaufile << "\n";

    HadTaufile << "# All the uncertainties in absolute numbers for both stat and syst uncertainties. Needed from QCD and hadronic tau; No need from lost lepton, Zinv and ttZ (will be ignored). For symmetric uncertainties, put the up and dn to be the same.\n";
    HadTaufile << "stat_unc_up = ";
    for(int i=0;i<NSB;i++){ HadTaufile << DC_sb_MC_HadTau_statunc[i] << " "; }
    HadTaufile << "\n";
    HadTaufile << "stat_unc_dn = ";
    for(int i=0;i<NSB;i++){ HadTaufile << DC_sb_MC_HadTau_statunc[i] << " "; }
    HadTaufile << "\n";

    HadTaufile << "# List of all the systematical uncertainties. Do not need combine them. The \"pdf\", \"blackhole\", \"darkmatter\", \"susy\" are keywords to label the different systematic sources (use meaningful names or make comments).\n";
    HadTaufile << "syst_unc_all_up = ";
    for(int i=0;i<NSB;i++){ HadTaufile << DC_sb_MC_HadTau_systunc[i] << " "; }
    HadTaufile << "\n";
    HadTaufile << "syst_unc_all_dn = ";
    for(int i=0;i<NSB;i++){ HadTaufile << DC_sb_MC_HadTau_systunc[i] << " "; }
    HadTaufile << "\n";
    HadTaufile.close();
  }
  ...
  if (Zinvfile.is_open())
  {
    ...
    Zinvfile << "channel = ";
    for(int i=0;i<NSB;i++){ Zinvfile << "bin" << i+1 << " "; }
    Zinvfile << "\n";
    
    Zinvfile << "# Predicted central numbers (need from all backgrounds)\n";
    Zinvfile << "rate = ";
    for(int i=0;i<NSB;i++){ Zinvfile << DC_sb_MC_Zinv[i] << " "; }
    Zinvfile << "\n";
    
    Zinvfile << "# Control sample events for lost lepton; Raw MC yields for Zinv and ttZ; No need from had. tau and QCD (will be ignored).\n";
    Zinvfile << "cs_event = ";
    for(int i=0;i<NSB;i++){ Zinvfile << DC_sb_MC_Zinv_cs[i] << " "; }
    Zinvfile << "\n";
    
    Zinvfile << "# Average weight for lost lepton, Zinv and ttZ: avg_weight x cs_event = rate. Will be ignored for had. tau and QCD.\n";
    Zinvfile << "avg_weight = ";
    for(int i=0;i<NSB;i++){ Zinvfile << DC_sb_MC_Zinv_avgweight[i] << " "; }
    Zinvfile << "\n";
    
    Zinvfile << "# All the uncertainties in absolute numbers for both stat and syst uncertainties. Needed from QCD and hadronic tau; No need from lost lepton, Zinv and ttZ (will be ignored). For symmetric uncertainties, put the up and dn to be the same.\n";
    Zinvfile << "stat_unc_up = ";
    for(int i=0;i<NSB;i++){ Zinvfile << /*DC_sb_MC_Zinv_statunc[i]*/"No Need!!" << " "; break; }
    Zinvfile << "\n";
    Zinvfile << "stat_unc_dn = ";
    for(int i=0;i<NSB;i++){ Zinvfile << /*DC_sb_MC_Zinv_statunc[i]*/"No Need!!" << " "; break; }
    Zinvfile << "\n";
    
    Zinvfile << "# List of all the systematical uncertainties. Do not need combine them. The \"pdf\", \"blackhole\", \"darkmatter\", \"susy\" are keywords to label the different systematic sources (use meaningful names or make comments).\n";
    Zinvfile << "syst_unc_all_up = ";
    for(int i=0;i<NSB;i++){ Zinvfile << DC_sb_MC_Zinv_systunc[i] << " "; }
    Zinvfile << "\n";
    Zinvfile << "syst_unc_all_dn = ";
    for(int i=0;i<NSB;i++){ Zinvfile << DC_sb_MC_Zinv_systunc[i] << " "; }
    Zinvfile << "\n";
    Zinvfile.close();
  }
  ...
  if (QCDfile.is_open())
  {
    ...
    for(int i=0;i<NSB;i++)
    {
      QCDfile << DC_sb_MC_QCD_systunc[i] << " ";
    }
    QCDfile << "\n";
    QCDfile.close();
  }
  ...
  if (TTZfile.is_open())
  {
    ...
    for(int i=0;i<NSB;i++)
    {
      TTZfile << DC_sb_MC_TTZ_systunc[i] << " ";
    }
    TTZfile << "\n";
    TTZfile.close();
  }
  ...
  if (Rarefile.is_open())
  {
    ...
    for(int i=0;i<NSB;i++)
    {
      Rarefile << DC_sb_MC_Rare_systunc[i] << " ";
    }
    Rarefile << "\n";
    Rarefile.close();
  }
